...
def render(df):
    ...
    # Nettoyage des données Intervals
    df_clean = df.copy()
    ...
    strat_label = get_coaching_strategy(ctl, atl, tsb, prof)
    st.success(strat_label)
    ...
    if render_sport_metric:
        ...
        c1.metric("Condition (CTL)", ctl)
        c2.metric("Fatigue (ATL)", atl)
        c3.metric("Forme (TSB)", tsb)

    # --- 2. GRAPHIQUE DOUBLE ETAGE (STYLE INTERVALS/GARMIN) ---
    st.markdown("### 📈 Analyse de la Condition Physique")
    
    # Création de deux lignes : 65% pour CTL/ATL, 35% pour TSB
    fig = make_subplots(
        rows=2, cols=1, 
        shared_xaxes=True, 
    ...
    # Condition (CTL) - Ligne Bleue épaisse
    fig.add_trace(go.Scatter(
    ...
    if race_date_raw:
        try:
            if not isinstance(race_date_raw, (date, datetime)):
                target_date = pd.to_datetime(str(race_date_raw))
            else:
                target_date = race_date_raw
    ...
    raw_data = prof.get('betrail_raw_data')
    if raw_data:
    ...
    # --- 4. DIAGNOSTIC IA ---
    st.markdown("---")
    feedback = get_ia_coaching_feedback(ctl, atl, tsb)
